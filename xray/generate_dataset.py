...
def argument_parser():
    ...
    images = os.listdir(os.path.join(args.images, f"{args.set}-set", "no_ooi", "zview"))
    ...
    if not args.multiview:
        ...
        parent_dir = os.path.join(args.output_dir, f"{args.set}-set")
        ...
        for id, label in tqdm(zip(ImageIds, labels), total=len(labels)):
            for view in views:
                ...
                src = os.path.join(args.images, f"{args.set}-set", src, f"{id}.png")
                dest = os.path.join(dest_dir, f"{id}.png")
                shutil.copy(src, dest)
